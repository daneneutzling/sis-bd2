...
CREATE TABLE `amizade` (
  ...
  `amigo_leitor_id` int DEFAULT NULL,
  UNIQUE KEY `amizade_uq` (`leitor_id`,`amigo_leitor_id`),
  KEY `amizade_leitor_amigo_fk` (`amigo_leitor_id`),
  CONSTRAINT `amizade_leitor_amigo_fk` FOREIGN KEY (`amigo_leitor_id`) REFERENCES `leitor` (`leitor_id`),
  CONSTRAINT `amizade_leitor_fk` FOREIGN KEY (`leitor_id`) REFERENCES `leitor` (`leitor_id`)
...
CREATE TABLE `autor` (
  ...
  UNIQUE KEY `autor_uq` (`nome`,`nascimento`,`nacionalidade`)
...
CREATE TABLE `livro` (
  ...
  UNIQUE KEY `livro_uq` (`nome`,`categoria`,`autor_id`),
...
-- Dump completed on 2022-05-10 16:40:44
